...
description: Delegate bounded coding, review, exploration, testing, or patch-generation tasks to external harnesses such as Claude Code, OpenCode, Codex CLI, or cheap model workers. Use when work can run in parallel, should preserve main context, or must be isolated into read-only or patch-only worker sandboxes.
---
...
6. Review any generated patch manually.
7. Apply patches only after policy checks pass:
...
- `read_only`: allow analysis only. The runner rejects jobs that modify files.
- `patch_only`: allow edits only in an isolated Git worktree. The runner saves `patch.diff` and checks it against path policy.
